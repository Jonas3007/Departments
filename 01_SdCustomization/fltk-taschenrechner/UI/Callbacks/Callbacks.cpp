...
static void chooseCalculator(const std::string& inputString, AppContext* ctx)
{
    if (!ctx || !ctx->calc || inputString.empty()) return;
...
void calculateResult_cb(Fl_Widget* /*w*/, void* data)
{
    ...
    ctx->history->AddtoTempHistory(inputString, ctx->calc->Result);

    // Ausgabe
    set_output(ctx, std::to_string(ctx->calc->Result));
}
